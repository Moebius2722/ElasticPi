...
sudo sed -i '/# node\.name: .*/a node.name: espi-node-1' /etc/elasticsearch/elasticsearch.yml
sudo sed -i '/# node\.rack: .*/a node.rack: espi-rack-1' /etc/elasticsearch/elasticsearch.yml
sudo sed -i '/# network\.host: .*/a network.host: localhost' /etc/elasticsearch/elasticsearch.yml
...
# Configure and Restart Reverse Proxy
